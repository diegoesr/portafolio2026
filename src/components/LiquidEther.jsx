  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container) {
        ...
        // Optimización: reducir pixelRatio para mejor rendimiento
        const isMobile = /(iPad|iPhone|iPod|Android)/i.test(navigator.userAgent);
        this.pixelRatio = isMobile ? 1 : Math.min(window.devicePixelRatio || 1, 1.5);
        this.resize();
        // Desactivar antialias para mejor rendimiento
...
`;
    ...
    class WebGLManager {
      ...
      loop() {
        ...
        this.render();
        // Usar requestAnimationFrame directamente sin throttling adicional
    ...
    const applyOptionsFromProps = (webglInstance) => {
      if (!webglInstance) return;
      const sim = webglInstance.output?.simulation;
      if (!sim) return;
    ...
    const setupObservers = (webglInstance) => {
      if (!webglInstance || !container) return;
    ...
    const initWebGL = () => {
      ...
      applyOptionsFromProps(webgl);
      webgl.start();
      setupObservers(webgl);
    };
